...
public class ShoppingReviewController {

    private final ReviewService reviewService;

    // 쇼핑 상세 페이지
    @GetMapping("/shopping/{id}")
    public String shoppingDetail(@PathVariable Long id, Model model) {
        Shopping shopping = reviewService.getShopping(id);
        model.addAttribute("shopping", shopping);
        return "shoppingDetail";
    }

    // 리뷰 전체 조회 (AJAX)
    @GetMapping("/shopping/{id}/review")
    @ResponseBody
    public ResponseEntity<List<ReviewDto>> getReviews(@PathVariable Long id) {
        List<ReviewDto> reviews = reviewService.ShoppinggetReviews(id)
                .stream()
    ...
    // 리뷰 작성 (AJAX)
    @PostMapping("/shopping/{id}/review")
    @ResponseBody
    public ResponseEntity<?> addReview(@PathVariable Long id,
        ...
        Review review = reviewService.ShoppingaddReview(id, principal.getName(), content);
        return ResponseEntity.ok(reviewService.toDto(review));
    }

    // 리뷰 수정 (AJAX)
    @PutMapping("/shopping/{id}/review/{reviewId}")
    @ResponseBody
    ...
    // 리뷰 삭제 (AJAX)
    @DeleteMapping("/shopping/{id}/review/{reviewId}")
    @ResponseBody
